...
void Kong();
void Dragon();
void PlayerAttack(Character player, int monster_type);
void EnemyAttack();
void FullScreen();
void GameTitle();
void TitleChoice();
...
{	
	...
	Monster slime;
		...
		slime.hp = 50;
		slime.max_hp = 50;
		slime.gold_drop = 50 + ((rand() % 10) - (rand() % 10));

	Monster wolf;
		wolf.attack = 15;
		wolf.hp = 100;
		wolf.max_hp = 100;
		wolf.gold_drop = 100 + ((rand() % 10) - (rand() % 10));

	Monster skeleton;
		skeleton.attack = 25;
		skeleton.hp = 75;
		skeleton.max_hp = 75;
		skeleton.gold_drop = 100 + ((rand() % 10) - (rand() % 10));
	
	Monster troll;
		troll.attack = 40;
		troll.hp = 150;
		troll.max_hp = 150;
		troll.gold_drop = 150 + ((rand() % 10) - (rand() % 10));
	
	Monster orc;
		orc.attack = 75;
		orc.hp = 250;
		orc.max_hp = 250;
		orc.gold_drop = 300 + ((rand() % 20) - (rand() % 20));
	...
	while (game == 0)
	{
		...
		while(story == 2.1) //Fight slime
		{
			...
			{
				printf("\"Beware of monster strong attack in %d turns.\"\n", monster_strong_attack);
			}
		...
		while(story == 7.44) //Receive quest to hunt troll
		{
			ClearScreen();
			...
			printf("You answer that you want to hunt some trolls. She silently nodded with a worried face, then spent some time looking for the paper and handing the quest paper to you.\n\n== You recieve troll hunt quest ==\n\n");
			printf("Type 1 to continue\n\n");
			printf("Type : ");
		...
		while(story == 7.441) //Found a group of troll
		{
			ClearScreen();
		...
		while(story == 7.4411) //Fight group of troll
		{
			ClearScreen();
...
void PrintInfo(Character player ,int wrong_chioce)
{
	...
	printf("%s\n",player.name);
		EndTextColor();
	printf("Heart %d | Health Point %d | Heal Power %d\n", player.heart, player.healthpoint, player.heal);
